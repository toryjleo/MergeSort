...
void MergeSort(int *A, int n) {
  ...
  for(i = 0; i < mid; ++i) {
    L[i] = A[i];   // Creates left subarray by iterating through the first half of the original array
  }
  for(i = mid; i < n; ++i) {
    R[i - mid] = A[i];                   // Creates right subarray by iterating through the right half of the original array
  }
